...
// generatePublicId — يولّد معرّف عام فريد بصيغة UUID v4 (36 حرف)
// التوافق مع SDK: TlvCodec يتوقع publicId بطول 36 بايت (UUID مع شرطات)
func generatePublicId() (string, error) {
	b := make([]byte, 16) // 16 بايت = 128 بت لـ UUID v4
	if _, err := rand.Read(b); err != nil {
		return "", fmt.Errorf("توليد معرّف عام: %w", err)
	}
	// ضبط إصدار UUID v4: البتات 4-7 من البايت 6 = 0100
	b[6] = (b[6] & 0x0f) | 0x40
	// ضبط المتغير RFC 4122: البتات 6-7 من البايت 8 = 10
	b[8] = (b[8] & 0x3f) | 0x80
	// تنسيق UUID: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx (36 حرف)
