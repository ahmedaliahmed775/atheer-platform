...
export default function TerminalPage() {
  ...
  useEffect(() => {
    ...
    const initTerminal = async () => {
      ...
      // تحويل عنوان API إلى WebSocket
      let wsUrl: string;
      if (switchUrl.startsWith("/")) {
        // مسار نسبي — بناء WebSocket من عنوان الصفحة الحالية
        const proto = window.location.protocol === "https:" ? "wss:" : "ws:";
        wsUrl = `${proto}//${window.location.host}${switchUrl}`;
      } else {
        // عنوان كامل — تحويل http → ws
        wsUrl = switchUrl.replace(/^http/, "ws");
      }
      wsUrl = wsUrl.replace(/\/$/, "");
