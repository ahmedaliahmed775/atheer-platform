...
type TransactionHandler struct {
        ...
        switchRecordRepo *repository.SwitchRecordRepository
}

// NewTransactionHandler creates a new TransactionHandler
func NewTransactionHandler(ts *service.TransactionService, ss *service.SagaService, srr *repository.SwitchRecordRepository) *TransactionHandler {
        return &TransactionHandler{txService: ts, sagaService: ss, switchRecordRepo: srr}
}

// Process handles POST /api/v2/transaction/
// Called by SDK: SideBProcessor sends CombinedRequest after NFC tap
// Pipeline already verified: AntiReplay → HMAC → PayeeType → Limits → TxType
func (h *TransactionHandler) Process(w http.ResponseWriter, r *http.Request) {
        packet := middleware.GetPayerPacket(r.Context())
        if packet == nil {
                response.BadRequest(w, response.ErrInternalError, "Packet not parsed")
                return
        }

        // Get payer record from context (set by SignatureVerifierA)
        payerRecord := middleware.GetPayerRecord(r.Context())
        // Get payee record from context (set by PayeeTypeVerifier)
        payeeRecord := middleware.GetPayeeRecord(r.Context())
        // Get transaction type from context (set by TransactionTypeResolver)
        txType := middleware.GetTransactionType(r.Context())
        ...
        result, err := h.txService.ProcessTransaction(r.Context(), packet, channel)
        if err != nil {
                slog.Error("Transaction creation failed",
                        "error", err,
                )
        ...
        // Execute Saga: Debit → Credit → SMS
        // In production, this should be async with status polling
        sagaReq := &service.SagaRequest{
                TxID:           result.TxID,
                Nonce:          fmt.Sprintf("%d", packet.Counter),
                SideAWalletID:  payerRecord.WalletID,
                SideAAccountID: payerRecord.UserID,
                SideADeviceID:  payerRecord.PublicID,
                SideBWalletID:  payeeRecord.WalletID,
                SideBAccountID: payeeRecord.UserID,
                SideBDeviceID:  payeeRecord.PublicID,
                TransactionType: txType,
                Currency:       packet.Currency,
                Amount:         decimal.NewFromInt(packet.Amount),
                Channel:        channel,
                SideACtr:       int64(packet.Counter),
        }

        sagaResult, err := h.sagaService.Execute(r.Context(), sagaReq)
        if err != nil {
                slog.Error("Saga execution failed",
                        "txId", result.TxID,
                        "error", err,
                )
                // Transaction already marked as FAILED by SagaService
                response.OK(w, result)
                return
        }

        slog.Info("Transaction processed via Saga",
                "txId", result.TxID,
