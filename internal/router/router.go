...
func New(deps *Dependencies) chi.Router {
        ...
        r.Route("/api/v2", func(r chi.Router) {
                ...
                // v3.0 Pipeline: Rate → Log → AntiReplay → HMAC → PayeeType → Limits → TxType → Handler
                // HMAC قبل LimitsChecker لمنع تسريب المعلومات وهجمات DoS
                r.Route("/transaction", func(r chi.Router) {
                        ...
                        r.Use(deps.SigVerifierA.Middleware)        // التحقق من HMAC — البند 3 [أولاً]
                        r.Use(deps.PayeeTypeVerifier.Middleware)   // التحقق من المستقبل — البند 4
                        r.Use(deps.LimitsChecker.Middleware)       // قيود الإنفاق — البند 1 [بعد HMAC]
                        r.Use(deps.TxTypeResolver.Middleware)      // تحديد TransactionType — البند 5
